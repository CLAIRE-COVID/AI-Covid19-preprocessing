...
def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    ...
    with torch.no_grad():
        ...
        X = torch.from_numpy(tvolslices).float().to(device).unsqueeze(dim=0)
        prediction = model(X)
...
def get_model(modeltype, modelname):
    ...
    fixed_model = os.path.join(sys.path[0], "unet_r231-d5d2fc3d.pth")
    state_dict = torch.load(fixed_model, map_location=torch.device('cpu'))
    if modeltype == 'unet':
